...
def QUERRY_getConsoPeriode(IDproduit): #renvoie le nb vendu sur la periode pour UN PRODUIT
    minutes = str(PERIOD_SIZE // 60)
    sec = str(PERIOD_SIZE % 60)
    if len(minutes) == 1:
        # on veut formater les minutes sur 2 chiffres
        minutes = "0"+minutes
    return ("SELECT count(*) "
            "FROM consos "
            "WHERE produits_id = {0} "
            "AND TIMEDIFF( NOW() , FROM_UNIXTIME(date)) < '{1}' ;".format(IDproduit, '00:'+minutes+':'+sec))
...
def QUERRY_getConsoTotalePeriode(): #renvoie le nb vendu sur la periode pour TOUS LES PRODUITS en jeu
    ...
    return ("SELECT count(*) "
            ...
            "AND TIMEDIFF( NOW() , FROM_UNIXTIME(date)) < '{1}' ;".format(SQLid_produit_jeu, '00:'+minutes+':'+sec))
...
def QUERRY_getConsoPeriodeMoinsUn(IDproduit): #renvoie le nb vendu sur la periode pour UN PRODUIT
    if time_period >= 10 :
        return ("SELECT count(*) "
                ...
                .format(IDproduit,
                        '00:'+str(2*time_period)+':00',
                        '00:'+str(time_period)+':00'))
    elif time_period >= 5:
        return ("SELECT count(*) "
                "FROM consos "
                "WHERE produits_id = {0} "
